<?php 
	...
	if(mysqli_num_rows($q)>0){
		while ($data = mysqli_fetch_assoc($q)) {
			$id_dmu_departemen[$index_dmu]=$data['id_departemen'];
			$index_dmu++;
		}
	}
	$index_dmu = 0;
	$index_dmu_ccr = 1;
	$n_dmu = count($id_dmu_departemen);
	...
	for ($y=0; $y < $n_dmu; $y++) { 
		$q1=mysqli_query($conn, 'SELECT d.id_variabel FROM detail_dmu AS d, variabel AS v WHERE d.id_variabel=v.id_variabel GROUP BY d.id_variabel ORDER BY v.jenis_variabel, v.id_variabel ASC');
		if (mysqli_num_rows($q1)>0) {
			...
			while ($d1=mysqli_fetch_assoc($q1)) {
				...
				if ($n_var <= $n_var_input) {
					...
					// Fungsi Z (var input)
					$q2=mysqli_query($conn, 'SELECT d.nilai_variabel FROM detail_dmu AS d, variabel AS v WHERE d.id_variabel=v.id_variabel AND d.id_variabel="'.$id_var.'" AND v.jenis_variabel="Input" ORDER BY d.id_variabel, d.id_departemen');
					...
					$q3=mysqli_query($conn, 'SELECT d.nilai_variabel FROM detail_dmu AS d, variabel AS v WHERE d.id_variabel=v.id_variabel AND d.id_variabel="'.$id_var.'" AND v.jenis_variabel="Output" ORDER BY d.id_variabel, d.id_departemen');
	...
	/*Menampilkan Hasil Efisiensi dan Rekomendasi*/
		...
		/*QUERY MEMASUKKAN HASIL PERHITUNGAN KE DATABASE*/
			...
			$nilai_awal = array();
			$query = mysqli_query($conn,'SELECT * FROM detail_dmu AS d, variabel AS v WHERE d.id_variabel = v.id_variabel AND v.jenis_variabel="Input" AND id_departemen="'.$id_dmu_departemen[$index_dmu].'" ORDER BY v.jenis_variabel, d.id_variabel');
			...
			for ($i=1; $i <= $n_var_input; $i++) { 
				...
				if($dual_simplex[1] < 1){
					...
					$q_insert= mysqli_query($conn, "INSERT INTO perhitungan_efisiensi (id_departemen, id_variabel, nilai_efisiensi, nilai_awal, rekomendasi) VALUES ('$id_dmu_departemen[$index_dmu]','$id_variabel[$ii]','$dual_simplex[1]','$nilai_awal[$ii]','$nilai_rekomendasi')");	
					...
					//jika efisiensi 1
					$q_insert= mysqli_query($conn, "INSERT INTO perhitungan_efisiensi (id_departemen, id_variabel, nilai_efisiensi, nilai_awal, rekomendasi) VALUES ('$id_dmu_departemen[$index_dmu]','$id_variabel[$ii]','$dual_simplex[1]','$nilai_awal[$ii]','$nilai_awal[$ii]')");
					/*echo "<br>Nilai awal efisiensi 1:<br>";
					print_r($nilai_awal);*/
